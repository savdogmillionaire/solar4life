...
class string(array):
    def __init__(self, number, model, panel_Voc, Voctemperaturecoeffcient_pc_per_C, panel_isc, wattage,panels,strings, str_number, length):
        array.__init__(self, number, model,panel_Voc, Voctemperaturecoeffcient_pc_per_C,panel_isc,wattage,panels,strings)
        self.str_number = str_number
        self.length = length

class inverter:
    def __init__(self,number,model, ac_breaker_current_limit,Max_AC_inverter_current,panel_startup_input_voltage,nominal_output,I_mppt_max,I_DC_max):
        self.number = number
        self.model = model
...
class systemspecs:
    ...
    def panel_Voc_minus_10(self):
        panel_1_Voc_minus_10 = self.array1.panel_Voc + (((self.array1.Voctemperaturecoeffcient_pc_per_C / 100) * -35) * self.array1.panel_Voc)
        panel_2_Voc_minus_10 = self.array2.panel_Voc + (((self.array2.Voctemperaturecoeffcient_pc_per_C / 100) * -35) * self.array2.panel_Voc)
        return (panel_1_Voc_minus_10, panel_2_Voc_minus_10)

    def array_Voc_minus_10(self):
        array_1_Voc_minus_10 =  (self.array1.panel_Voc + (((self.array1.Voctemperaturecoeffcient_pc_per_C / 100) * -35) * self.array1.panel_Voc)) * self.array1.panels
    ...
    def inverter_mppt_max_check(self):
        ...
        if (array_isc1 and array_isc2) < self.inverter1.I_DC_max and (array_isc1 and array_isc2) < self.inverter1.I_mppt_max:
            return "GOOD"
        if (array_isc1 and array_isc2) < self.inverter1.I_DC_max and (array_isc1 and array_isc2) > self.inverter1.I_mppt_max:
            return "KINDA BAD"
        if (array_isc1 and array_isc2) > self.inverter1.I_DC_max and (array_isc1 and array_isc2) > self.inverter1.I_mppt_max:
    ...
    def Power_optimiser_limit(self):
        ...
        return "Number of Panels less than or equal to allowed based on %.2fW inverter oversizing: %d" % (oversize1, answer)
